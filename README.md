...
Так как используется один `airflow-worker` и `worker_concurrency = 16`, worker теоретически может взять до 16 задач одновременно. При этом глобальный лимит `parallelism = 32`, то есть текущий стенд скорее ограничен worker capacity, а не глобальным `parallelism`.
...
Это похоже не на индивидуальные лимиты контейнеров, а на общий лимит памяти, доступный Docker Desktop.
